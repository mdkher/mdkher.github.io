...
var _getScrollFunc = function _getScrollFunc2(element, _ref) {
  ...
  func.target = element;
  prev ||
    (func.smooth = gsap.getProperty(element, "scrollBehavior") === "smooth");
  return func;
};
...
var Observer = (function () {
  ...
  _proto.init = function init(vars) {
    ...
    lineHeight ||
      (lineHeight = parseFloat(_win.getComputedStyle(_body).lineHeight) || 22);
    var id,
      onStopDelayedCall,
      dragged,
      moved,
...
var _getGSAP3 = function _getGSAP4() {
  return (
    gsap2 ||
    (_windowExists() && (gsap2 = window.gsap) && gsap2.registerPlugin && gsap2)
  );
};
...
var _onResize = function _onResize2(force) {
  ...
  (force === true ||
    (!_refreshing &&
      !_ignoreResize &&
      !_doc2.fullscreenElement &&
      !_doc2.webkitFullscreenElement &&
      (!_ignoreMobileResize ||
        _baseScreenWidth !== _win2.innerWidth ||
        Math.abs(_win2.innerHeight - _baseScreenHeight) >
          _win2.innerHeight * 0.25))) &&
...
var _clearScrollMemory = function _clearScrollMemory2(
  ...
  _isString(scrollRestoration) &&
    (_win2.history.scrollRestoration = _scrollRestoration = scrollRestoration);
};
var _refreshingAll;
var _refreshID = 0;
var _queueRefreshID;
...
var ScrollTrigger2 = (function () {
  ...
  _proto.init = function init(vars, animation) {
    ...
    var _vars = vars,
      ...
      scroller = _getTarget(vars.scroller || _win2),
      scrollerCache = gsap2.core.getCache(scroller),
    ...
    if (snap) {
      ...
      "scrollBehavior" in _body2.style &&
        gsap2.set(isViewport ? [_body2, _docEl2] : scroller, {
          scrollBehavior: "auto",
        });
      _scrollers.forEach(function (o) {
        return (
          _isFunction(o) &&
          o.target ===
            (isViewport ? _doc2.scrollingElement || _docEl2 : scroller) &&
          (o.smooth = false)
        );
      });
          ...
          if (
            ...
            var progress = (scroll - start) / change,
              ...
              velocity = refreshedRecently
                ? 0
                : ((totalProgress - snap2) / (_getTime2() - _time2)) * 1e3 || 0,
              change1 = gsap2.utils.clamp(
                -progress,
                1 - progress,
    ...
    if (markers) {
      ...
      containerAnimation &&
        (caMarkerSetter = gsap2.quickSetter(
      ...
      if (
        ...
        markerStartSetter = gsap2.quickSetter(
          markerStartTrigger,
          direction.a,
          _px,
        );
    ...
    self.refresh = function (soft, force, position, pinOffset) {
      ...
      if (isFirstRefresh) {
        prevProgress = gsap2.utils.clamp(
          0,
          1,
          gsap2.utils.normalize(start, end, prevScroll),
        );
      }
        ...
        animation &&
          ...
          animation.totalProgress(
            containerAnimation && start < -1e-3 && !prevProgress
              ? gsap2.utils.normalize(start, end, 0)
              : prevProgress,
            true,
          );
        self.progress =
  ...
  ScrollTrigger3.enable = function enable() {
    ...
    if (gsap2) {
      ...
      if (_body2) {
        ...
        var bodyHasStyle = _body2.hasAttribute("style"),
          bodyStyle = _body2.style,
          border = bodyStyle.borderTopStyle,
          AnimationProto = gsap2.core.Animation.prototype,
          bounds,
          i;
  ...
  ScrollTrigger3.scrollerProxy = function scrollerProxy(target, vars) {
    ...
    if (vars) {
      isViewport
        ? _proxies.unshift(_win2, vars, _body2, vars, _docEl2, vars)
  ...
  ScrollTrigger3.isInViewport = function isInViewport(
    ...
    return horizontal
      ? bounds.right - offset > 0 && bounds.left + offset < _win2.innerWidth
      : bounds.bottom - offset > 0 && bounds.top + offset < _win2.innerHeight;
  };
  ScrollTrigger3.positionInViewport = function positionInViewport(
    ...
    return horizontal
      ? (bounds.left + offset) / _win2.innerWidth
      : (bounds.top + offset) / _win2.innerHeight;
  };
...
ScrollTrigger2.refresh = function (safe) {
  return safe
    ? _onResize(true)
    : (_coreInitted2 || ScrollTrigger2.register()) && _refreshAll(true);
};
...
var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
  ...
  var _vars2 = vars,
    ...
    scale = 1,
    initialScale =
      (Observer.isTouch && _win2.visualViewport
        ? _win2.visualViewport.scale * _win2.visualViewport.width
        : _win2.outerWidth) / _win2.innerWidth,
    wheelRefresh = 0,
    ...
    updateClamps = function updateClamps2() {
      ...
      normalizeScrollX &&
        (scrollClampX = _clamp2(0, _maxScroll(target, _horizontal)));
  ...
  vars.onPress = function () {
    ...
    var prevScale = scale;
    scale = _round(
      ((_win2.visualViewport && _win2.visualViewport.scale) || 1) /
        initialScale,
    );
    tween.pause();
    prevScale !== scale &&
...
export { ScrollTrigger2 as ScrollTrigger, ScrollTrigger2 as default };
