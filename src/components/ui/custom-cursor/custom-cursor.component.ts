...
@Component({
  ...
  template: `
    <div
      #cursor
      class="fixed top-0 left-0 w-3 h-3 bg-white rounded-full pointer-events-none z-[100] mix-blend-exclusion transition-transform duration-100 ease-out will-change-transform"
    ></div>

    <div
      #preview
      class="fixed top-0 left-0 w-64 h-40 pointer-events-none z-[90] overflow-hidden opacity-0 scale-50 rounded-lg origin-center will-change-transform"
    >
      <img
        *ngIf="cursorService.previewImage() as src"
        [src]="src"
...
export class CustomCursorComponent implements AfterViewInit {
  ...
  constructor() {
    effect(() => {
      ...
      const hasImage = !!this.cursorService.previewImage();

      if (isPlatformBrowser(this.platformId) && this.cursor && this.preview) {
        if (isHovering && hasImage) {
