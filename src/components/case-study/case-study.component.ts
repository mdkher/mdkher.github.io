...
export class CaseStudyComponent implements AfterViewInit, OnDestroy {
  ...
  activeSection = toSignal(
    fromEvent(window, "scroll").pipe(map(() => this.detectActiveSection())),
  ...
  ngAfterViewInit() {
    ...
    if (this.stickyHeader && this.heroTitle) {
      ScrollTrigger.create({
        trigger: this.heroTitle.nativeElement,
        start: "bottom top", // when bottom of title hits top of viewport
        onEnter: () =>
          gsap.to(this.stickyHeader.nativeElement, {
            y: "0%",
            duration: 0.3,
            ease: "power2.out",
          }),
  ...
  renderImpactChart(project: Project) {
    ...
    svg
      ...
      .call((g: any) =>
        g
          .select(".domain")
          .attr("stroke", this.themeService.isDark() ? "#333" : "#e2e8f0"),
      )
      .selectAll("text")
      .attr("fill", this.themeService.isDark() ? "#888" : "#64748b")
      .attr("dy", "1.5em")
